...
//		list.add(5,"소방차ㅋㅋ"); //런타임 에러 발생
		...
		/*
		중복저장
			List는 배열의 특성을 가지므로 데이터를 중복 저장할 
			수 있다. 동일한 데이터라도 index로 구분할 수 있기때문이다. 
			add()는 추가에 성공할 경우 true를 반환한다. */
		System.out.println(list.add("트와이스") ? "중복저장 됨" : " 중복저장 안됨");
		...
		방법2 : 확장 for문의 경우 인덱스없이 전체 원소를 반복해주므로
			좀더 간단한 표현이 가능하다. 따라서 가장 많이 사용된다.	 */
		System.out.println("\n[확장for문 사용]");
		for(String obj : list) {
		...
		*/
		...
		/*
		방법2 : 방법1과 동일하게 인덱스로 삭제한다.
				단, indexOf로 인스턴스의 인덱스를 찾은 후
				이 값을 사용한다. */
