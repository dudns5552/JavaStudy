...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		객체저장
			저장시 기존에 저장된 동일한 Key값이 존재하면 저장된 인스턴스가
			반환된다. 만약 처음이라면 null을 반환한다.
		 */
...
//		map.put("age", number);
		...
		/*
		중복저장
			기존에 입력된 name이라는 Key가 있기때문에 '홍길동'이 반환된다.
			그리고 기존의 값은 '최길동'으로 수정된다.
		*/
		System.out.println("name이라는 키값으로 저장된 이전의 값 : "+
				map.put("name", "최길동"));
		//기존의 값을 덮어쓰기 하므로 4개가 출력된다.
		System.out.println("키값으로 중복 저장한 후의 객체의 수 : "+
				map.size());
		...
		/*
			3. 이터레이터를 통한 출력
				...
				1) Map의 전체 Key를 얻어온 후
				2) 이터레이터 인스턴스를 생성
				3) Key가 있는지 확인 후
				4) 존재하면 Key를 얻어와서 value를 인출한다.*/
		
		System.out.println("[이터레이터 사용하기]"); // 1)
		Set<String> keys2 = map.keySet(); // 2)
		Iterator<String> it = keys2.iterator(); // 3)
		while(it.hasNext()) { //4)
		...
		/*
		존재유무확인
			Map은 Key와 vlaue가 있으므로 메서드도 2개로 나눠져있다. */
		...
		/*
		전체삭제
			removAll()은 없고, clear()만 사용할 수 있다.
		 */
		map.clear();
