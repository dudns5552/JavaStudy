...
HashSet<E> : set계열의 컬렉션
	...
	- 인스턴스가 순서없이 저장된다.
	- 중복저장을 허용하지 않는다.
	- 기본클래스(String, Integer 등)인 경우에는 별도의 처리없이 중복이
		제거된다.
	- 하지만 개발자가 정의한 클래스라면 hashCode(), equals()라는 메서드를
		오버라이딩 해야한다.
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		이터레이터를 통한 객체 출력
			...
			2. hasNext()인출할 데이터가 있는지 확인
			3. true가 반환되면 next()를 통해 인출한다. */
		Iterator itr = set.iterator(); //1
		while(itr.hasNext()) { //2
			Object object = itr.next(); //3
			
			/*
			HashSet에 저장시 모두 Object형으로 자동형변환되어 저장되므로
			우리가 직접 정의한 클래스에 대해서만 instanceof를 통해
			타입을 확인한 후 다운캐스팅[((C)Obj)]하여 출력한다.
			만약 다운캐스팅을 하지않으면 자식멤버에 접근할 수 없으므로
			getName을 호출할 수 없다. */
			if(object instanceof Teacher) {
				System.out.println("Teacher객체 =>"+ 
					((Teacher)object).getName()); //다운캐스팅
			}
			else {
				//기본클래스인 경우 별도의 처리없이 출력이 가능함.
		...
		/*
		객체삭제(인스턴스 삭제)
			set은 인덱스를 사용하지 않으므로 인스턴스의 참조값을
			통해 삭제한다 */
