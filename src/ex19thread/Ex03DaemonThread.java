...
독립쓰레드(Non daemon thread)
	...
	종료되는 쓰레드를 말한다. 해당 쓰레드는 main()메서드가 종료된 이후에도
	계속 실행되는 것을 볼 수 있다. */
class NormalThread extends Thread {
	...
	public NormalThread() {}
	/*
	Thread클래스의 생성자 호출시 전달하는 인수를 통해 이 쓰레드의 이름을
	지정할 수 있다. 여기서 지정된 이름은 getName()을 통해 확인할 수 있다. */
	public NormalThread(String title) {
		super(title);
	}
	/*
	쓰레드 동작중 비정상적인 종료가 될때 InterruptedException이 
	발생할 수 있으므로, 예외처리를 해야한다. 여기서는 try~catch로 직접 
	처리하지않고, throws해서 호출한 위치로 예외던지기를 하고 있다. */
...
종속쓰레드(Daemon thread)
	...
	종료될때까지는 프로그램이 종료되는 것이 아니다.
	주로 프로그램의 보조역할로 배경음악재생, 자동저장 등의 업무를 처리한다.
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		않았으므로 5로 지정된다. 쓰레드 인스턴스 생성시 이름을 부여하는
		방법으로 아래 2가지가 있다. */
