...
- 추상클래스는 대부분 추상메서드를 포함하고 있으므로, 이를 상속받은 
...
추상메서드
	- 오버라이딩 목적으로 제작된다.
	- 메서드의 실행부가 없으므로 중괄호 대신 세미클론(;)으로 마무리한다.
 */
...
abstract class AbsClass1 {
	...
	public static final int
		MAX_INT = Integer.MAX_VALUE; //멤버 변수
	//일반적인 멤버메서드
	void instanceMethod() {}
	//정적 메서드
...
class AbsClassChild2 extends AbsClass2 {
	/*
	오버라이딩은 부모 쪽의 메서드를 가리고, 새로운 기능을 재정의 하는
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		static의 기본규칙을 따른다. 별도의 생성 절차없이 바로 클래스명으로
		접근 할 수 있다. */
