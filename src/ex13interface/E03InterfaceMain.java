...
인터페이스(interface)
	...
	- 추상클래스와 동일하게 상속을 목적으로 제작되므로 구현받은 
		하위클래스에서는 반드시 추상메서드를 오버라이딩 해야한다.
	- 멤버로는 추상메서드와 상수만 선언할 수 있다.(밑의 형태가 자동으로 붙는다.)
		메서드 : public abstract
		상수 : public static final
	- 인터페이스는 생성자를 선언할 수 없다. */
interface MyInterface1 {
	
	//생성자(constructors) 선언 불가
//	public MyInterface1 () {}
	
	/*
	멤버상수 : 인터페이스에 선언되는 변수는상수로 선언된다.
			 따라서 대문자로 작성해야된다.
...
class MyClass extends SimpleClass 
	...
	오버라이딩 하면 된다.(absFunc2) 오버라이딩은 항상 자식쪽의  
	메서드가 호출되므로 아무런 문제도 발생하지 않는다.
	 */
	@Override
	...
	/*
	SimpleClass 클래스의 멤버메서드(mySimple)는 오버라이딩이 필수가 아니므로
	필요한 경우에만 추가하면된다.
	 */
...
public class E03InterfaceMain {
	...
	public static void main(String[] args) {
		...
		/*
		'구현'한 인터페이스를 통해 참조변수를 선언할 수 있다. MyClass의
		부모는 모두 3개이다. */
