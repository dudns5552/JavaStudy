...
상속(Inheritance)
	...
	정의하는 것을 말한다. 자식클래스가 부모클래스를 상속할때 extends를
	사용한다. */
...
class BusinessMan extends Man {
			...
			String position) 
	{
		...
		호출해준다. 만약 super()를 기술하지 않으면 컴파일러에 의해
		자동으로 삽입되어 디폴트 생성자를 호출하게된다. */
		
		//부모클래스의 매개변수 1개인 생성자 호출(메모리에 먼저 생성됨)
		super(name); //super 함수호출 문장이라고 보면됨
	...
	public void tellYourInfo() {
		...
		상속받은 자식클래스라 하더라도 접근 할 수 없다. private멤버는
		무조건 자신의 클래스 내부에서만 접근할 수 있다. */
//		System.out.println("이름은 " + name + " 입니다.");
		/*
		직접적인 접근이 안되므로 public으로 선언된 메서드를 통해
		간접적으로 접근해야한다. */
		tellYourName();
	}
}
