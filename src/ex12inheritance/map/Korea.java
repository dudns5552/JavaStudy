...
public class Korea {
	...
	public void publicMethod(String loc) {
		...
		/*
		default로 선언된 메서드는 패키지가 다른 클래스에서는 호출할
		수 없으므로 해당 메서드를 통해 간접 호출해야한다. */
		defaultMethod("publicMethod -> ");
	}
	protected void protctedMethod(String loc) {
		/*
		서로 패키지가 다른 2개의 클래스가 상속관계를 가지고 있다면
		접근을 허용한다. 즉 default 접근지정자보다 넓은 접근범위를
		가진다. */
		System.out.println(loc + "protctedMethod() 호출");
		privateMethod("protctedMethod -> ");
	}
	void defaultMethod(String loc) {
		/*
		패키지가 동일할때만 접근을 허용한다. 패키지가 다른 경우 상속
		관계가 아니라면 public으로 선언해야 접근할 수 있다. */
		System.out.println(loc + "defaultMethod() 호출");
	}
	/*
	동일 클래스 내부에서만 접근을 허용한다. 즉 멤버 이외에는 아무도 접근
	할 수 없다. */
	private void privateMethod(String loc) {
		System.out.println(loc + "pivateMethod() 호출");
	}
}
