...
class Account {
	...
	//입금처리 : protected로 선언되었으므로 상속관계에선 접근가능
...
class SavingAccount extends Account {
	...
	public SavingAccount(int initVal) {
		/*
		이 문장을 상제갛면 에러가 발생하게된다. 삭제할 경우 
		super()라는 문장, 즉 매개변수가 없는 디폴트 생성자를 
		호출하는 문장이 자동으로 추가되는데, 부모클래스에는 이런 형태의 
		생성자가 없으므로 에러가 발생하는 것이다. */
		super(initVal);
	}
	...
	public void showAccountMoney() {
		/*
		잔고확인을 위한 멤버메서드로 부모의 멤버인 money에 직접 접근할 수
		없으므로 아래와 같이 getter메서드로 반환받아 출력해야한다. */
...
//		sa.money = 100000; //에러발생
		...
		변수를 통해서도 money에 접근할 수 없다. 인스턴스 생성 위치가
		외부 클래스이기 때문이다. 즉 private 멤버는 무조건 클래스
		내부에서만 접근을 허용한다. */
