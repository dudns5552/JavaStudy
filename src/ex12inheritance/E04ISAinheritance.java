...
class Computer{
	...
	//디폴트 생성자 (매개변수 실행부가 없음)
//	public Computer() {}
	
	/*
	매개변수가 있는 인자생성자 추가. 이 경우 디폴트 생성자는 
	추가되지 않는다. */
...
컴퓨터를 상속한 노트북
	...
	휴대하기 위해서는 배터리가 필요하므로 멤버변수로 확장하였다. */
...
class TabletNotebook extends NotebookComputer{
	...
	public void write(String penInfo) {
		...
		A.comparTo(B) 는 일치하는 경우 0을 반환한다. */
		if(registPencil.compareTo(penInfo) != 0) {
			System.out.println("등록된 펜이 아닙니다.");
			return;
		}
//		if(registPencil.equals(penInfo) == false) {
//			System.out.println("등록된 펜이 아닙니다.");
//			return;
//		}
		...
		단, 접근이 가능한 지정자로 선언되어 있어야한다.
		super는 생략 할 수 있다.(하지만 자식클래스의 멤버메서드가 아니란것을
		명시적으로 나타내기위해 사용하는게 좋다.) */
		super.movingCal(); 
...
public class E04ISAinheritance {
	...
	public static void main(String[] args) {
		...
		System.out.println("==XYZ-5678 펜으로 테블릿 사용==");
		tablet.write("XYZ-5678"); //사용불가
	}
