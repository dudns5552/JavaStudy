...
싱글톤 디자인 패턴
	...
	할당한 후 이를 프로그램 전체에서 공유해서 사용할 때 쓰는 패턴
	
작성방법
	1. 생성자의 접근지정자를 pirvate으로 선언
...
class NoSingleTone {
	int instVar;
	/*
	생성자는 거의 대부분 public으로 선언한다. 만약 public으로 선언하지
	않으면 클래스 외부 혹은 다른 패키지에서는 호출할 수 없기 때문에
	new를 통해 인스턴스 생성 자체가 불가능하기 때문이다. */
	public NoSingleTone() {}
}
...
class SingleTone {
	...
	int shareVar;
	/*
	정의한 클래스와 동일한 타입으로 정적 멤버변수로 JVM(자바가상머신)에
	의해 프로그램이 시작될때 미리 Method영역에 로드되어 사용할 준비를
	마치게 된다. */
	...
	/*
	정적메소드로 선언된 getINstance()를 통해 해당 인스턴스의 참조값을
	외부로 반환한다. 이런 메서드를 "유틸리티(유용한) 메서드"라고 한다. */
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		"not visible"즉, 보이지 않는다. 라는 에러가 발생한다. */
