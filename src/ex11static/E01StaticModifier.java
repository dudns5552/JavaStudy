...
class MyStatic {
	...
	/*
	멤버변수 접근방법 : 인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한
		후 참조변수를 통해 접근해야한다. 즉, 일반적인 멤버변수는 인스턴스를
		생성하는 시점에 메모리에 로드된다.
		(멤버변수는 Stack영역에, 인스턴스는 Heap영역에 생성됨) */
	int instanceVar; 
	
	/*
	정적변수 접근방법 : 인스턴스를 생성할 필요없이 클래스명으로 바로 접근할 수
		있다. JVM(자바가상머신)이 프로그램을 시작할때 미리
		메모리에(Method영역)에 로드된다. */
	static int staticVar; 
	
	/*
	인스턴스형 멤버메서드 : 인스턴스형 메서드에서는 모든 멤버를 사용할 수 있다.
		인스턴스형 멤버, 정적 멤버 모두 접근이 가능하다. */
	void instanceMethod() { //일반메서드
	...
	정적 멤버메서드 : 정적메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다.
		정적 멤버만 접근이 가능하다.
		(※ main 메서드에서는 static으로 선언된 메서드만 호출할 수 있었던
		것을 기억하자.) */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		인스턴스 변수로 접근하든 동일한 메모리가 된다. 따라서 값을 공유하게
		되므로 동일한 결과를 출력한다.	 */
		ms1.staticVar = 800;
		ms2.staticVar = 900;
		...
		//이와같이 작성하면 누가 보더라도 같은 변수임을 한눈에 알 수 있다.
...
//		MyStatic.instanceMethod(); //에러
		...
		/*
		결론
			1. 일반적인 인스턴스형 멤버는 인스턴스의 참조변수를 통해
			접근한다.
			2. 정적(static) 멤버는 클래스명을 통해 인스턴스
			생성없이 접근한다. */
