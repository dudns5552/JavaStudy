...
public class Ex02PreDefineException1 {
	...
	public static void main(String[] args) {
		...
		발생되면 JVM(자바가상머신은)은 해당 예외에 관련된 인스턴스를
		만들어 프로그램쪽으로 전달한다.
		이때 catch절에서 예외 인스턴스를 잡아 주는 것을 "예외처리"라고
		한다.
		만약, 이 예외가 catch되지 못하면 프로그램은 비정상적인 종료로
		이어지게 된다. */
		try {
			intArr [0] = 100;
			...
			System.out.println("예외메세지1 : 예외가 발생했어요");
			//2 : 간략한 설명을 출력
			System.out.println("예외메세지2 : "+ e.getMessage());
			//3 : 예외에 대한 디테일한 설명을 출력(가장 많이 사용됨)
			System.out.print("예외메세지3 : ");
			e.printStackTrace();
		}
