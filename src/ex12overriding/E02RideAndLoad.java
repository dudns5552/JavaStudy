...
오버로딩 (다른메서드)
	메서드명은 같지만 매개변수의 타입, 갯수, 순서가 다른 경우를
	말한다. 즉, 서로 다른 메서드이므로 상속받은 하위클래스에는 오버로딩된
	모든 메서드가 포함된다.

오버라이딩 (같은메서드)
	메서드명, 매개변수, 반환형까지 모두 동일한 메서드를 상속관계에
	있는 하위클래스에서 재정의 한것을 말한다.
...
class C extends B {
	...
	/*
	load메서드는 오버로딩으로 정의했으므로 메서드명만 동일할뿐
	매개변수의 타입, 갯수 등이 다르다. 따라서 서로 다른 메서드이다. */
	public void loadMethod(double num) {
		System.out.println("C의 loadMethod");
	}
	/*
	따라서 C클래스로 인스턴슬르 생성하면 오버라이딩된 메서드는 모두
	가려지므로 1개만 남게되고, 오버로딩된 메서드는 3개 모두 존재하게된다.
	즉, C인스턴스에는 4개의 메서드가 존재한다. */
}
...
public class E02RideAndLoad {
	...
	public static void main(String[] args) {
		...
		접근할 수 있다. 따라서 아래 3개의 문자은 모두 실행된다. */
		ref3.loadMethod(); //a
		ref3.loadMethod(1); // b
		...
		ref2.loadMethod(); //a
		ref2.loadMethod(1); //b
//		ref2.loadMethod(1.1); //에러발생 접근할 수 없음

		System.out.println();
		
		//A타입으로는 A클래스까지만 접근된다.
		ref1.loadMethod(); //a
//		ref1.loadMethod(1); //에러발생 접근할 수 없음
//		ref1.loadMethod(1.1); //에러발생 접근할 수 없음
