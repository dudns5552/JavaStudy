...
public class DeChild extends DeParent {
	...
	public void study() {
		...
		getter메서드를 호출해서 값을 간접적으로 받아야한다. */
//		System.out.println(name + "이(가) 공부한다.");
	...
	/*
	오버라이딩 불가
		부모클래스에서 private멤버로 선언 하였으므로	자식쪽에서는 
		보이지 않아 오버라이딩의 대상이 될수 없다.이 메서드는 
		자식쪽에서	확장한 메서드가 된다. */
	private void eat() {
	...
	- 부모클래스에 선언될때와 비교해서 접근범위가 동일하거나
		그보다 넓으면 가능하다.
	- 부모에서 default로 선언하면 자식에게는 dafault 혹은
	...
	*/
	...
	/*
	static(정적)메서드는 오버라이딩의 대상이 될 수 없다. 클래스 외부
	(메서드영역)에 미리 로드되므로, 오버라이딩 되지않고 호출할 때 각
	클래스명을 사용하면 된다. */
