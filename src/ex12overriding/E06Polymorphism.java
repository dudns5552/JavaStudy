...
class MyChild extends MyParent {
	...
	void parentMethod(int childMember) {
		...
		System.out.println("Overloading : 자식에서 확장한"
				+ "메소드 : parentMethod(param1)");
	}
}
...
//		parent1.childMember = 1;
		...
		//부모 참조변수로 자식인스턴스를 참조한 두번째 인스턴스 생성
...
//		(MyChild)myParent.childMember = 1; //잘못된 형변환
		소괄호를 하나더 추가해서 myParent를 자식타입으로 형변환 후
		멤버변수에 접근해야 한다.(위에처럼 하면 형변환이 애매하게 인식됨)
		 */
		((MyChild)myParent).childMember = 1; //올바른 형변환
		...
		/*
		Java에서 생성한 모든 클래스는 Object클래스를 상송한다. 따라서
		모든 인스턴스는 Object의 참조변수로 참조가 가능하다. 또한
		Object 클래스에 정의된 모든 메서드를 별도의 선언없이 사용할 수
