...
public class E05Date {
	...
	public static String myDateFormat
		(Calendar c, String formatStr) {
		/*
		서버의 표준시인 1970-01-01 부터 현재까지의 시간을 초 단위로
		변환해주는 메서드로 이를 '타임스템프'라고 한다. */
		Date date = c.getTime();
...
//		System.out.println(today+ " "+ future);
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			따라서 각 날짜의 서식을 명확히 지정해야만 날짜를 인식하여
			연산할 수 있다. */
			//각 서식의 날짜를 파싱해서 Date 인스턴스로 변환한다.
			Date formatToday = new SimpleDateFormat("yyyy-MM-dd")
					.parse(today);
			Date formatFuture = new SimpleDateFormat("yyyy/MM/dd")
					.parse(future);
