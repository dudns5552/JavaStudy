...
class Car {
	...
	장성한 클래스는 객체를 표현하기 위해 생성하지만, Java에서는 하나의
	자료형으로 사용된다. 즉 객체를 추상화 한 또 하나의 자료형을 생성하는
	것이 class를 만드는 목적이라 할 수 있다.*/
	...
	/*
	초기화 메서드2 : 위와 동일한 이름의 메서드로 오버로딩에 의해 정의되어있다.
		초기화를 진행하는 역할은 동일하나, 매개변수를 통해 다양한 값으로
		초기화 할 수 있다.
	 */
	void initialize(String model, String name, int age, int energy) {
		...
		생성하지 않은 상태이므로, 초기화를 위해 인스턴스를 생성한 후 초기화를
		진행 하고있다.*/
		owner = new Human();
		owner.name = name;
		owner.age = age;
	...
	void drive() {
		/*
		owner는 인스턴스 이므로 이름을 출력하기 위해 변수명.name과
		같이 기술한다.
		 */
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		자동차2 객체 생성 : 초기화 메서드를 호출하는 대신 멤버변수에
			...
			생성할 때마다 동일한 코드를 작성해야 하므로 비효율적이다.
