...
public class E01JunsuType {
	...
	- 일반적으로 CPU(중앙처리장치)는 정수형 연산을 가장 빠르게 처리하는
	  특징이 있다. 즉 int에 최적화 되어있다.
	- 따라서 정수형 변수를 사용할 때에는 대부분 int형을 사용하면된다.
	 */
	public static void main(String[] args) {
		...
		/*
		자바는 정수형 상수를 기본적으로 int형으로 간주하기 때문에 아래 정수를
		메모리에 로드하는 순간 에러가 발생된다.
		이때 접미사(L 혹은 l)를 붙여서 int형의 자료가 아닌 long형의 자료임을
		표시해야 한다.
		이와같이 입력되는 숫자도 자료형을 기반으로 메모리에 로드되는데 이를 가르켜
		리터럴(Literal)이라고 한다.
		 */
