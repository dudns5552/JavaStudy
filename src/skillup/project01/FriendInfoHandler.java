...
public class FriendInfoHandler {
	...
	/*
	연락처를 저장하기 위한 인스턴스형 배열을 생성. 고딩/대딩 각가의
	...
	//생성자 : 배열의 크기를 결정할 값(num)을 매개변수로 받아서 초기화
	public FriendInfoHandler(int num) {
		//num의 크기로 배열이 생성된다.
		highFriends = new HighFriend[num];
		//입력된 정보의 갯수를(or 인덱스) 카운트하기 위한 변수
		numOfhighFriend = 0;
...
//		System.out.println("## deleteIno 호출됨 ##");
		...
		/*
		삭제될 인스턴스의 인덱스 값을 저장하기 위한 변수 배열의 인덱스는
		...
		if(deleteIndexHigh != -1) {
			//고딩친구 정보가 삭제되었을 때 배열 정리
			for(int i = deleteIndexHigh ; i < numOfhighFriend;
					i++) {
				/*
			...
			//대딩친구 정보가 삭제되었을 때 배열 정리
			for(int i = deleteIndexUniv ; i < numOfunivFriend;
					i++) {
			...
			//고딩/대딩 모두 -1을 유지한다면 일치하는 데이터가 없다는 의미
